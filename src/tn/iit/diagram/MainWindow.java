...
public class MainWindow extends JFrame {
	...
	public MainWindow() {
		super("Class Diagram of " + DBConnection.login);
		setVisible(true);
	...
	private void createTables() {
		for (String tableName : tables) {
			try {
				...
				TableNode _table = diagram.getFactory().createTableNode(10, 10, 50, tableData.size() * 8, 4,
						tableData.size());
	...
	private void arrangeDiagram() {
		...
		for (DiagramLink link : diagram.getLinks()) {
			...
			if (link.getOrigin().getBounds().getX() < link.getDestination().getBounds().getX()) {
				System.out.println(link.getDestination().getBounds().x + "    " + link.getEndPoint().y);
